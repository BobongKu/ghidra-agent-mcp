  ...
  const runQueue = async (queue: UploadJob[]) => {
    ...
    for (const j of queue) {
      ...
      try {
        ...
        const r = await tauri.uploadBinary(j.path, serverUrl, analysis);
          ...
          prev.map((x) =>
            x.id === j.id
              ? {
                  ...
                  message: ok
                    ? `${fns ?? "?"} fns`
                    : r.error ?? r.message ?? "import failed",
                  size: r.size ?? 0,
                  finishedAt: Date.now(),
                }
              : x
